<template>
    <div>
        <ul class="emojiTab">
            <li v-for="(pannel,index) in pannels" @click="changeActive(index)" :key="index"  :class="{active:index == activeIndex}">
                {{pannel}}
            </li>
        </ul>
  ...
  computed: {
    emojis () {
      return this.pannels.map(item => {
        return Object.keys(this.emojiData[item])//属性数组
      })
    }
  },
};
...
<style>
@import url('../assets/css/emoji-sprite.scss');
